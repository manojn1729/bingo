...
function createBox(){
  ...
  for(let y=0;y<5;y++){
    for(let x=0;x<5;x++){
      box.push(new Box(x*boxSize,y*boxSize,boxSize,boxSize,arr[y*5+x]));
    }
  }
}
...
function drawInput(){
  playerInput=createInput('Enter Name').position(windowWidth/3, windowHeight/10);
  gameId=createInput('Enter game Id').position(windowWidth/3, windowHeight*2/10);
  playerSubmit=createButton('Join').position(windowWidth/3, windowHeight*3/10);
  playerSubmit.mousePressed(sendPlayerDetails);
}
